...
- release：拖动过 → 回 IDLE（并补最后一帧位置）；否则视为点击 →
  仅发 pet_clicked（未来行为扩展点；V1 单击无动作，HAPPY 由右键「拍一拍」触发）。

拖动候选机制（防止快速点按被按住期抖动误判为拖动）：
- 点按时按住瞬间常有 5~40px 漂移，若越过阈值就立即判为拖动，点按会被吞掉。
- 因此越过阈值先进入“拖动候选”：① 位移 ≥ TAP_MAX_DRIFT_PX 立即拖动；
  ② 否则等 TAP_MAX_HOLD_MS，期间松开 → 按点击结算（仅发 pet_clicked）；
...
TAP_MAX_HOLD_MS = 300  # 候选期按住 ≤300ms 松开 → 视为快速点按（仅发 pet_clicked）
TAP_MAX_DRIFT_PX = 40  # 位移超过该值 → 立即判定为拖动（不等候选期）
POLL_MS = 16           # 拖动跟随的光标轮询间隔（≈60fps）


class InteractionController(QObject):
    state_requested = Signal(str)   # "DRAG" | "IDLE"（HAPPY 由右键「拍一拍」经 FSM 触发）
    ...
    def on_mouse_release(self, global_pos: QPoint) -> None:
        ...
        if self._dragging:
            ...
            return  # 真拖动：不算点击
        # 未正式拖动（含候选期快速松开的“点按”）→ 单击：仅发扩展信号，无动作
        self._drag_candidate = False
        self.pet_clicked.emit(global_pos)

    def _start_drag(self) -> None:
        """正式进入拖动：请求 DRAG，启动跟随轮询。"""
        if not self._pressed or self._dragging:
            return
        self._dragging = True
        self._drag_timer.stop()
